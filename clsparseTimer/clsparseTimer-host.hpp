...
class clsparseHostTimer: public clsparseTimer
{
    //	Private typedefs
     ...
     */
    clsparseHostTimer( const clsparseHostTimer& );

    /**
     * \fn operator=( const StatisticalTimer& )
     * \brief Assignment operator does not make sense for a singleton, disallow assignments
     */
    clsparseHostTimer& operator=( const clsparseHostTimer& );

    friend std::ostream& operator<<( std::ostream& os, const clsparseHostTimer& s );
...
public:
     ...
     */
    static clsparseHostTimer& getInstance( );
    ...
    size_t pruneOutliers( size_t id, cl_double multiple );
};
