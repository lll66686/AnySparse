...
cl_int fill(Container& c, const T & value, cl::CommandQueue& queue)
{
    ...
    if (c.size() > 0)
    {
#if defined(BUILD_CLVERSION) && (BUILD_CLVERSION >= 120)
      status = queue.enqueueFillBuffer(c.data(), value, 0,
                                         c.size() * sizeof(T),
                                         NULL, &controlEvent);
#else
#if 0
      // 在OpenCL 1.1中，使用enqueueWriteBuffer来模拟填充功能
      std::vector<T> fillData(c.size(), value);
      
      status = queue.enqueueWriteBuffer(queue, c.data(), CL_TRUE, 0,
                                    c.size() * sizeof(T),
                                    fillData.data(), 0, NULL, &controlEvent);
#endif
#endif
...
cl_int fill(Container& c, const T & value)
{
    ...
    {
#if defined(BUILD_CLVERSION) && (BUILD_CLVERSION >= 120)
        status = c.getQueue().enqueueFillBuffer(c.data(), value, 0,c.size() * sizeof(T),NULL, &controlEvent);
#else
      // 在OpenCL 1.1中，使用enqueueWriteBuffer来模拟填充功能
      std::vector<T> fillData(c.size(), value);
#if 0
      status = c.getQueue().enqueueWriteBuffer(c.getQueue(), c.data(), CL_TRUE, 0, c.size() * sizeof(T),fillData.data(), 0, NULL, &controlEvent);
#endif
#endif
