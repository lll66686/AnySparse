...
#include <cstdarg>
  ...
  static void
ocl_sparse_log2 (const char *fmt, ...)
{
  FILE *fp = fopen ("/Users/tjc/Downloads/ocl1_clsparse_log.txt", "a");
...
clsparseCreateControl( cl_command_queue queue )
{
	...
	cPair.control->addressBits = 32; // default 64 bits
cPair.control->pDeviceTimer = NULL;

    collectEnvParams( cPair.control );

    //	Discover and load the timer module if present
    void* timerLibHandle = NULL;//LoadSharedLibrary( "lib", "clsparseTimer", false );
    if( timerLibHandle )
    {
